...
interface DataContextProps {
    ...
    nodesDataArray: go.ObjectData[];
    linksDataArray: go.ObjectData[] | undefined;
...
interface Signature {
    ...
    instances: {
        [id: string]: { // represents the id of the instance, unique for all function calls
...
export const DataProvider = ({ children }: any) => {
    ...
    const [nodesDataArray, setNodesDataArray] = useState<go.ObjectData[]>([]);
    const [linksDataArray, setLinksDataArray] = useState<go.ObjectData[] | undefined>([]);
    const [treeNodes, setTreeNodes] = useState<go.ObjectData[]>([]);
    const [treeLinks, setTreeLinks] = useState<go.ObjectData[]>([]);
    const [gridNodes, setGridNodes] = useState<go.ObjectData[]>([]);
    ...
    const runCode = () => {
        ...
        const mockData: Data = {
            ...
            signatures: {
                "module": {
                    numInstances: 1, totalTime: 1700,
                    instances: {
                        "0": { caller: "", line: 0, time: 1700 }
                    }
                },
                "fib(n=5)": {
                    numInstances: 1, totalTime: 1400,
                    instances: {
                        "11": { caller: "0", line: 4, time: 1400, returnValue: 8 }
                    }
                },
                "fib(n=4)": {
                    numInstances: 1, totalTime: 900,
                    instances: {
                        "1": { caller: "11", line: 4, time: 900, returnValue: 5 }
                    }
                },
                "fib(n=3)": {
                    numInstances: 1, totalTime: 1000,
                    instances: {
        ...
        const treeNodes: go.ObjectData[] = [];
        const treeLinks: go.ObjectData[] = [];
        const gridNodes: go.ObjectData[] = [];
        ...
        for (const signature in mockData.signatures) {
            ...
            if (signature !== root) {
                gridNodes.push({
                    ...
                    totalTime: totalTime,
                    color: `#${convert.hsv.hex([120 - (totalTime / runtime) * 120, 100, 100])}`,
            ...
            for (const instanceId in mockData.signatures[signature].instances) {
                const { caller, time, line, returnValue } = mockData.signatures[signature].instances[instanceId];
                treeNodes.push({
                    key: instanceId,
                    label: signature,
                    line: line,
